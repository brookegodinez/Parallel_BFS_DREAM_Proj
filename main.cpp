        ...
        {    
            ...
            for(int j = 0; j < k; j++)
	        {
                ...
                //already been visited i.e. is the dist != -1 
                {
                    ...
                    tmp_weights[E[offset[curr_v]+j]] = curr_dist - 1; //different
                    flag[j] = 1;
                    //add to frontier
                }
                else 
...
void new_BFS(int n, int m, int* offset, int* E, int s, int* dist)
{
    ...
    int p = 2;
    for(int i = 0; i < n; i++) temp_new_offset[i] = pre_processing_BFS(n, m, offset, E, i, dist, p, new_neighbors, weights);
 

    new_offset = exclusive_scan(temp_new_offset, n);
    new_offset[n] = inclusive_scan(temp_new_offset, n)[n-1];
    ...
    BFS(n, new_offset[n], new_offset, new_E, 0, dist, new_weights);
